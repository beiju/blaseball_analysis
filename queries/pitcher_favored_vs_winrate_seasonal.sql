-- i just learned about CTEs whoo boy it sure seems like they were the wrong tool for this job
-- judging from the fact that this query is slow as crabitat word
...
pitcher_records as (select
	player_name,
	sn as season,
	(select count(*) from game_records where favored_pitcher_id=player_id and season=sn) as times_favored,
	(select count(*) from game_records where underdog_pitcher_id=player_id and season=sn) as times_underdog,
	(select count(*) from game_records where winning_pitcher_id=player_id and season=sn) as times_won,
	(select count(*) from game_records where losing_pitcher_id=player_id and season=sn) as times_lost
from data.players
cross join generate_series(0, 23) as sn
