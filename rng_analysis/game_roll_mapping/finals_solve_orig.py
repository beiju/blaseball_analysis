...
def strike_swing_check(pitcher, batter, outcome):
    ...
    if outcome in ["b"] and strike_pred:
        print(
            "ERROR @ {}: rolled {}, too low for ball, threshold is {}".format(
                r.get_state_str(), strike_roll, strike_threshold(pitcher_ruth)
            )
        )
...
flyout("Theodore Cervantes", "Fish Summer", "Conrad Vaughan", inning_ending=True)
...
ball("Nagomi Meng", "Thomas Dracaena")
bird()
ground_out("Nagomi Meng", "Thomas Dracaena", "Jessica Telephone", inning_ending=True)
...
flyout(
    "Nagomi Meng", "Winnie Mccall", "Peanutiel Duffy", runner=True, inning_ending=True
)
...
ground_out("Nagomi Meng", "Dominic Marijuana", "Moody Cookbook", inning_ending=True)
...
ground_out(
    "Theodore Cervantes", "Alyssa Harrell", "Thomas Dracaena", inning_ending=True
)
...
ball("Nagomi Meng", "Schneider Bendie", runner=True)
ground_out(
    "Nagomi Meng", "Schneider Bendie", "Ren Morin", runner=True, inning_ending=True
)
...
r.step(14)  # sandie turner fc, runner on first and second, definitely exactly 14

bird()
ground_out(
    "Nagomi Meng", "Dominic Marijuana", "Fish Summer", runner=True, inning_ending=True
)
